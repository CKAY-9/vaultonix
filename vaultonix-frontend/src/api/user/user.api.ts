...
import { TradeDTO } from "../vaultonix/vaultonix.dto";
...
export const getUserFromGuild = async (
  guild_id: string,
  user_id: string
): Promise<GuildUserDTO | null> => {
  try {
    const request = await axios({
...
export const getUserFromVaultonix = async (
  user_id: string,
  guild_id: string
