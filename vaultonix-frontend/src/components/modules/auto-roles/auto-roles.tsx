  ...
  useEffect(() => {
    (async () => {
      if (
        ...
        props.guild_config.join_roles === undefined
      ) {
        return;
      }
      ...
      setRoles(
        r.filter((role) => !props.guild_config.join_roles.includes(role.id))
      );
      setAutoRoles(
        r.filter((role) => props.guild_config.join_roles.includes(role.id))
      );
  ...
  return (
    <>
      ...
      <div className={style.add_role}>
        <button onClick={() => setShowRoles(!show_roles)}>
          {show_roles ? "Hide" : "Show"} Roles
        </button>
        {show_roles && (
