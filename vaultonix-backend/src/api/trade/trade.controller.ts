...
import { NewTradeDTO } from './trade.dto';

@Controller("")
export class TradeController {
  @Post("")
  async createNewTrade(@Req() request: Request, @Body() body: NewTradeDTO, @Res() response: Response) {
    ...
    if (!flag) {
      ...
      if (user_check === null) {
        return response.status(HttpStatus.NOT_FOUND).json({message: "Failed to find user."});
      }
    }
    ...
    if (partner_check === null) {
      return response.status(HttpStatus.NOT_FOUND).json({message: "Failed to find partner."});
    }
