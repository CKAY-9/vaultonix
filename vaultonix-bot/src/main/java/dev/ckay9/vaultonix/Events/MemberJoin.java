...
public class MemberJoin extends ListenerAdapter {
    ...
    public void onGuildMemberJoin(GuildMemberJoinEvent event) {
        try {
            ...
            CloseableHttpResponse response = HTTP.getRequest(Vaultonix.API_HOST + "/guild/config?guild_id=" + guild.getId(), new Header[]{
            ...
            CloseableHttpResponse welcome_response = HTTP.getRequest(Vaultonix.API_HOST + "/guild/welcome_goodbye?guild_id=" + guild.getId(), new Header[]{
                new BasicHeader("Authorization", Vaultonix.BOT_AUTH_KEY)
            });
            HttpEntity welcome_entity = welcome_response.getEntity();
            WelcomeResponse welcome_parsed = gson.fromJson(EntityUtils.toString(welcome_entity, "UTF-8"), WelcomeResponse.class);
            if (!welcome_parsed.enabled) return;
            if (welcome_parsed.channel_id.length() <= 0) return;
