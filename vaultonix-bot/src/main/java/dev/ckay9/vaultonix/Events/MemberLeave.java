...
public class MemberLeave extends ListenerAdapter {
    ...
    public void onGuildMemberRemove(GuildMemberRemoveEvent event) {
        try {
            ...
            CloseableHttpResponse goodbye_request = HTTP.getRequest(Vaultonix.API_HOST + "/guild/welcome_goodbye?guild_id=" + guild.getId(), new Header[]{});
            HttpEntity goodbye_entity = goodbye_request.getEntity();
            Gson gson = new Gson();
            GoodbyeResponse goodbye_response = gson.fromJson(EntityUtils.toString(goodbye_entity, "UTF-8"), GoodbyeResponse.class);
            if (!goodbye_response.enabled) return;
            if (goodbye_response.channel_id.length() <= 0) return;
